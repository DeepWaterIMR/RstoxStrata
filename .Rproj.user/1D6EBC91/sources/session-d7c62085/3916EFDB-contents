...
##' @param nrow,ncol Number of rows and columns when \code{facetted = TRUE}. See \code{\link[ggplot2]{facet_wrap}}.
...
plot.strataPolygon <- function(x, basemap = TRUE, facetted = FALSE, fill_color = "red", fill_alpha = 0.8, ncol = NULL, nrow = NULL, ...) {
    ...
    if(facetted) {
      ggOceanMaps::basemap(data = x$geostrata, ...) +
        ...
        ggplot2::geom_sf_text(data = suppressWarnings(sf::st_centroid(x$geostrata)),
                              ...
                              fontface = 2) +
        ggplot2::facet_wrap(~ interval, ncol = ncol, nrow = nrow)
    } else {
      ggOceanMaps::basemap(data = x$geostrata, ...) +
