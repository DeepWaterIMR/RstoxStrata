...
These information can then be input into the `strataPolygon()` function which performs the strata estimation. The function returns a `strataPolygon` object, which is a list containing **strata** and **geostrata** [sf](https://cran.r-project.org/web/packages/sf/index.html) polygon objects. 
...
Next we compare this strata system to the depth-based system.

# Results

## Difference in area

```{r, areadiff, class.source = 'fold-hide', fig.cap = "bla"}
x <- dplyr::full_join(
...
x$pr.diff <- 100*(x$area.nm2-x$orig.nm2)/x$orig.nm2

p1 <- ggplot(x, aes(x = orig.nm2, y = area.nm2)) +
...
p2 <- ggplot(x, aes(x = paste(geostrata.name, interval, sep = "\n"), y = pr.diff, fill = interval)) +
geom_col() +
scale_fill_viridis_d("Depth strata") +
labs(x = "", y = "Percentage difference (negative = larger original area)", fill = "Strata ID") +
theme_bw() +
...
Make map, improve the figures. Make table percentage difference instead of the figure? Write something about the area difference
