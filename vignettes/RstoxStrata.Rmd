...
```{r, differenceplot, echo = FALSE, fig.cap = "Bottom depth at start of trawling for stations that did not get assigned to the same depth strata between depth-latitude- and position-based assignement. Position-based strata are shown on the x-axis together with the depth interval (grey bar). Dots represent stations. The color of dots indicates position-based strata."}
...
Number of differently assigned stations / year
...
The difference between original and reconstructed strata areas on the index based on depth- and latitude station assignment was influenced by outliers. Strata with only a few stations per year had a high impact on the result  

Difference between old and new area, depth based and position based assignment figure

```{r, echo = FALSE, fig.cap = "Blup"}

bind_rows(
db_index %>% 
  group_by(startyear) %>% 
  summarise(value = sum(index)) %>% 
  mutate(type = "No removal") %>% 
...
db_index %>% 
  ...
  labs(x = "Year", y = "Biomass (1000 tons)", color = "Number of\nstations\nremoved") +
  theme_bw() +
  theme(#legend.position = "bottom",
